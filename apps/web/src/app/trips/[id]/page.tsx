...
export default function TripDetailPage() {
  ...
  useEffect(() => {
    if (params.id) {
      Promise.all([
        api.getTrip(params.id as string),
        api.listWardrobe(),
      ])
        .then(([t, w]) => {
          setTrip(t);
  ...
  const handleGenerateOutfits = useCallback(async () => {
    ...
    setGenError('');
    try {
      const updated = await api.generateOutfits(trip.id);
      setTrip(updated);
    } catch (err) {
      setGenError(err instanceof ApiRequestError ? err.message : 'Failed to generate outfits');
    } finally {
      setGenerating(false);
    }
  }, [trip, generating]);
  ...
  const handleSwap = useCallback(async (newItemId: string) => {
    ...
    try {
      ...
      const updated = await api.getTrip(trip!.id);
      setTrip(updated);
      setSwapTarget(null);
    } catch (err) {
  ...
  // Packing list: dedupe wardrobe items across all outfits
  const packingList = (() => {
    if (!trip?.days) return [];
      ...
      {loading ? (
        ...
        <div className="animate-fade-in">
                    ...
                    <div key={day.id} className="card">
                      ...
                      {hasData ? (
                        ...
                        <div className="mb-4 rounded-lg border border-dashed border-border px-3 py-3 text-center text-xs text-accent-dim">
                          No weather data —{' '}
                          <button onClick={handleRefreshWeather} disabled={refreshing} className="text-accent-muted hover:text-accent transition-colors">
                            fetch weather
                          </button>
                        </div>
                      )}
                      ...
                      {dayOutfits.length > 0 ? (
                        <div className="space-y-3">
                          {dayOutfits.map((outfit) => (
                            <OutfitCard key={outfit.id} outfit={outfit} onSwap={(oi) => setSwapTarget({ outfit, outfitItem: oi })} />
                          ))}
                        </div>
                      ) : (
                        <div className="rounded-lg border border-dashed border-border px-3 py-5 text-center text-xs text-accent-dim">
                          No outfits —{' '}
                          <button onClick={handleGenerateOutfits} disabled={generating} className="text-accent-muted hover:text-accent transition-colors">
                            generate outfits
                          </button>
                        </div>
                      )}
          ...
          <Modal open={!!swapTarget} onClose={() => setSwapTarget(null)} title="Swap item">
            {swapTarget && (
              <div>
                <p className="mb-1 text-xs text-accent-dim">
                  Replace: <span className="text-accent-muted">{swapTarget.outfitItem.wardrobeItem?.name}</span>
                </p>
                <p className="mb-4 text-xs text-accent-dim">
                  Category: {CATEGORY_LABELS[swapTarget.outfitItem.wardrobeItem?.category ?? ''] ?? 'Unknown'}
                </p>
...
function OutfitCard({ outfit, onSwap }: { outfit: Outfit; onSwap: (item: OutfitItem) => void }) {
    ...
    <div className={`rounded-lg border p-4 ${isNight ? 'border-indigo-500/20 bg-indigo-500/[0.03]' : 'border-border bg-surface-subtle/50'}`}>
      <div className="mb-3 flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <span className="text-xs font-semibold uppercase tracking-wider text-accent-muted">
      ...
      <div className="mb-3 space-y-1.5">
        {(outfit.items ?? []).map((oi) => (
          <div key={oi.id} className="group flex items-center gap-2.5 rounded-md bg-surface-raised/50 px-3 py-2">
            ...
            <button
              onClick={() => onSwap(oi)}
              className="rounded px-2 py-1 text-[10px] text-accent-dim opacity-0 group-hover:opacity-100 hover:bg-surface-subtle hover:text-accent-muted transition-all"
            >
              Swap
            </button>
          </div>
      ...
      {bullets.length > 0 && (
        <details className="group">
          ...
          <ul className="mt-2 space-y-0.5">
            {bullets.map((b, i) => (
              <li key={i} className="text-[11px] text-accent-dim leading-relaxed">{b}</li>
            ))}
          </ul>
        </details>
      )}
